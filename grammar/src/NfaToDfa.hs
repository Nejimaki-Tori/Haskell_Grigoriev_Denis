...
allIter nfa alph pd pB qB tr
                ...
                | otherwise = do
                        ...
                        let queue'' = if Set.member qd qB then pB else if not (Set.null qd) then Set.insert qd pB else pB
                        let newQSet = if Set.member qd qB then qB else if not (Set.null qd) then Set.insert qd qB else qB
